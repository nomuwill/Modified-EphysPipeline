...
- **MQTT job listener** – Schedules Kubernetes jobs in response to MQTT topics defined in an S3-hosted CSV, with built-in chaining so one job’s completion event can trigger the next.【F:Services/Spike_Sorting_Listener/README.md†L5-L35】【F:Services/Spike_Sorting_Listener/README.md†L23-L46】
  ...
  - **MaxTwo handling** – Sorting jobs that run `./run.sh` include a MaxTwo splitter init container. The init container runs the maxtwo splitter, exits cleanly if the dataset is not `maxtwo`/`max2`, and otherwise uploads split wells to `s3://braingeneersdev/ephys/<UUID>/original/split/`. The sorter uses the cached `braingeneersdev` path when metadata reports `maxtwo`/`max2`; otherwise it reads from the standard `braingeneers` bucket.
